...
SC_MODULE(axi_slave_if) {
  ...
  //---------------------------------//
  // 解析读请求包，并构造读请求发送给slave
  void rd_req_depack_job () {
    // 用于记录已发送请求但未收到响应的事务个数
    sc_uint<LOG_MAX_OUTS>  rd_in_flight =  0;
    // 用于记录前一个事务id，用于outstanding。
    ...
    while(1) {
      ...
      if(rd_flit_in.PopNB(flit_rcv)) {
        ...
        #pragma hls_pipeline_init_interval 1
        #pragma pipeline_stall_mode flush
        // 这里的策略应该是：如果当前事务和前一个未完成事务id不同，则需要等待前一个事务完成后才能发送；
        // 或者当未完成事务大于2个，sc_fifo<rd_trans_info_t>已经满时，也需要等待之前事务完成之后才能发送
        ...
        // 检查slave的总线宽度是否比AxSIZE小，若小的话，应该进行resize，即减少AxSIZE，增加AxLEN
        ...
        temp_req.addr  = ((((flit_rcv.data[2] >> dnp::req::AH_PTR) & ((1 << dnp::AH_W) - 1)) << dnp::AL_W) |
                           ((flit_rcv.data[1] >> dnp::req::AL_PTR) & ((1 << dnp::AL_W) - 1))) - slave_base_addr.read();
        ...
        rd_in_flight++;
        // 记录当前事务id，用于outstanding策略
        outst_tid = temp_info.tid;
        
        // 将info放入fifo中存放，以便验证响应；
  ...
  void rd_resp_pack_job () {
    ...
    while(1) {
      ...
      // 响应按序返回，因为相同事务id的事务需要保证按序返回。rd_trans_init保存的是之前在解析请求包时保存的信息，可以用于构造header flit
